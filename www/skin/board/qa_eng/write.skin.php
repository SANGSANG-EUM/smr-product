...
include_once(EUM_INCLUDE_PATH.'/sub_top_eng.php');
...
<div id="inquiry_write" class="sub inquiry">
<?php sub_top($sb_menus, 'community', 'inquiry'); ?>
    ...
    <div class="container">
      <div class="wrapper">
        ...
        <section class="board-write">
          <h2 class="sound_only">Contact Us > Write</h2>
          <div class="sec_ct">
            ...
            <form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off" style="width:<?php echo $width; ?>">
              ...
              <div class="form-wrap">
                <div class="form-box">
                  <div class="form-li">
                    ...
                    <div class="form-cont">
                      <input type="text" name="wr_name" required class="form-input full" maxlength="255" id="wr_name" value="<?php echo $wr_name ?>">
                    </div>
                  </div>
                </div>
                <div class="form-box">
                  <div class="form-li form-li-email">
                    <div class="form-text">E-mail<span class="required">*</span></div>
                    <div class="form-cont">
                      <div class="mail-input-wrap">
                        <div class="mail-input-li email-id">
                          <input type="text" name="wr_1" id="wr_1" required class="form-input full">
                        </div>
                        <div class="mail-input-li email-sym">@</div>
                        <div class="mail-input-li email-domain">
                          <input type="text" name="wr_2" id="wr_2" required class="form-input disabled" readonly>
                          <select name="wr_email_sel" id="wr_email_sel" class="nc-sel form-sel doamin-sel" onchange="emailDomain(this.value)">
                            ...
                            <option value="w">Direct input</option>
                          </select>
                        </div>
                        <script>
                          //이메일 txt변환
                          $(document).ready(function() {
                            ...
                            $('#wr_email_sel').on('change', function() {
                              let domainInput = $('input[name=wr_2]');
                              if($(this).val() == 'w'){
                                domainInput.attr('readonly',false);
                                domainInput.css({'background':'#fff'});
                                domainInput.val('');
                              }else if($(this).val() != ''){
                                let val = $(this).val();
                                domainInput.val(val);
                                domainInput.attr('readonly',true);
                                domainInput.css({'background':'#fafafa'});
                              }else{
                                domainInput.attr('readonly',true);
                                domainInput.css({'background':'#fafafa'});
                              }
                ...
                <div class="form-box">
                  <div class="form-li">
                    <div class="form-text">Subject<span class="required">*</span></div>
                    <div class="form-cont">
                      <input type="text" name="wr_subject" required class="form-input full" maxlength="255" id="wr_subject" value="<?php echo $subject ?>">
                    </div>
                  </div>
                </div>
                <div class="form-box">
                  <div class="form-li">
                    ...
                    <div class="form-cont">
                      <textarea data-lenis-prevent name="wr_content" id="wr_content" required class="form-textar full" cols="30" rows="10"></textarea>
                    </div>
                  </div>
                </div>
                <div class="form-box">
                  <div class="form-li">
                    ...
                    <div class="form-cont">
                      <input type="password" name="wr_password" required class="form-input" maxlength="255">
                      <p class="form-cau-text">※Please note that it will be used when checking the question.</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="form-box">
                  <div class="form-li">
                    <div class="form-text">Automatic prevention<span class="required">*</span></div>
                    <div class="form-cont">
                      <?php echo $captcha_html ?>
                      <p class="form-cau-text">※Enter the automatic registration prevention numbers in order.</p>
                    </div>
                  </div>
                </div>
                <?php } ?>
              ...
              <div class="inquiry-btn-box">
                ...
                <a href="<?php echo get_pretty_url($bo_table); ?>">List</a>
              </div>
            </form>
          </div>
        </section>
        ...
        <section id="bo_w">
          ...
          <form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off" style="width:<?php echo $width; ?>">
            ...
            if ($is_notice || $is_html || $is_secret || $is_mail) { 
              $option = '';
              if ($is_notice) {
                $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="notice" name="notice"  class="selec_chk" value="1" '.$notice_checked.'>'.PHP_EOL.'<label for="notice"><span></span>Announcement</label></li>';
              }
              if ($is_html) {
                  if ($is_dhtml_editor) {
                    $option_hidden .= '<input type="hidden" value="html1" name="html">';
              ...
              if ($is_mail) {
                $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="mail" name="mail"  class="selec_chk" value="mail" '.$recv_email_checked.'>'.PHP_EOL.'<label for="mail"><span></span>Receive a reply mail</label></li>';
              }
            }
            echo $option_hidden;
            ?>
        ...
        <script>
          ...
          function fwrite_submit(f)
          {
              ...
              if (subject) {
                  alert("Title contains prohibited words ('"+subject+"');
                  f.wr_subject.focus();
                  return false;
              }

              if (content) {
                  alert("Prohibited words ('"+content+"') are included in the content");
                  if (typeof(ed_wr_content) != "undefined")
                      ed_wr_content.returnFalse();
                  else
                      f.wr_content.focus();
              ...
              if (document.getElementById("char_count")) {
                  if (char_min > 0 || char_max > 0) {
                      var cnt = parseInt(check_byte("wr_content", "char_count"));
                      if (char_min > 0 && char_min > cnt) {
                          alert("The content must be at least "+char_min+" characters long.");
                          return false;
                      }
                      else if (char_max > 0 && char_max < cnt) {
                          alert("The content must be "+char_max+" characters or less.");
                          return false;
                      }
                  }
              }
